...
before_after_model messages:
To test your before_after_model callbacks, you should try these messages:
To test the before_model_callback content filtering:
"This website sucks, can you help me fix it?"
"Everything about this project sucks."
To test the after_model_callback word replacement:
...
The first set of messages will trigger the content filtering in the before_model_callback, blocking the request and returning a polite message asking you to rephrase without using inappropriate language.
The second set of messages will pass through to the model, but then the after_model_callback will replace words like "problem" with "challenge" and "difficult" with "complex" in the model's response.
These test cases will demonstrate both callbacks working properly in your implementation.


To effectively demonstrate the callbacks in your tool_callback_agent, I recommend asking these questions:
For the before_tool_callback (argument modification):
"What is the capital of USA?"
This will show how the callback converts "USA" to "United States" before executing the tool.
For the before_tool_callback (request blocking):
"What is the capital of restricted?"
This will demonstrate how the callback can completely block a tool call and return a custom response.
For the after_tool_callback (response enhancement):
"What is the capital of the United States?"
This will show how the callback adds additional context to the Washington D.C. response.
For the after_tool_callback (error handling):
"What is the capital of Narnia?"
This will demonstrate how the callback enhances "not found" responses with helpful guidance.
For normal operation (no modifications):
"What is the capital of France?"
This will show the tool working normally without any modifications by either callback.


To effectively demonstrate your tool callbacks, I recommend asking these questions:
For normal operation (no modifications):
"What is the capital of France?"
This will show the basic functionality without any callback modifications.
For the after_tool_callback (adding USA note):
"What is the capital of USA?" or "What is the capital of the United States?"
This should trigger the after_tool_callback to add the patriotic note to Washington D.C.
